...
- **HTTP 目录索引**：ASCII 横幅 + 信息框（*** 区域）+ 目录/文件列表，与 *** 区域同宽（80ch）的布局
- **Web 上传**：选择文件后「上传文件」可用，多文件上传，成功后刷新当前目录
- **新建文件**：弹框输入文件名与可选内容，在当前目录创建文件
- **新建文件夹**：弹框输入文件夹名，在当前目录创建目录
- **文件下载**：支持中文等文件名（URL 解码），点击即下载
