...
- 样式优化：改进文件列表布局，添加 favicon 支持
- 术语规范化：统一使用「新建文件夹」等措辞
- 布局调整：优化内容区域和上传框的最大宽度
- 配置简化：移除 .gitkeep 相关规则，简化 .gitignore
...
- 新建文件夹：弹框输入文件夹名，POST `/api/newdir`，创建后重定向；同名目录返回 400
- 布局：顶部 *** 区域与底部操作区、分割线统一为 80ch 最大宽度；上传/新建文件/新建文件夹三按钮右对齐、间距 1em
